batch_spikes <- function(rawdata, std_factor = 1, min_isi = 1, sigma = NA) {
	#' \code{batch_spikes} a first try on copying the functionality of the matlab H1 toolbox batch_spikes.m function
	...
	filenames <- dir()
	filenames <- filenames[1:ncol(rawdata)]
	# rawdata <- rawdata[2]
	...
	spikes <- array(data = NA_real_, dim = c(nrow(rawdata), ncol(rawdata)))
	...
	for (i in 1:length(filenames)) {
		# the function simplifies the data structure and returns a data frame
		# signal <- get_H1signal(rawdata[, i])
		signal <- rawdata[, i]
